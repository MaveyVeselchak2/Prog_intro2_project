class Board:
    """
    Игровое поле 10x10 для морского боя.

    Внутри доска хранится как двумерный список чисел:
    0 - пустая клетка / вода
    1,2 - целая часть корабля, число = уникальный ID корабля
    -1,-2 - подбитая часть корабля, число = отрицательный ID корабля
    -99 - промах
    """
    SIZE = 10
